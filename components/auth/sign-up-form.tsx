...
export interface SignUpFormProps { onSuccess?: () => void }
/** Sign-up form using Convex password provider. */
export function SignUpForm({ onSuccess }: SignUpFormProps): ReactElement {
  ...
  const onSubmit = async (): Promise<void> => {
    ...
    try { await signIn("password", { flow: "signUp", email, password }); onSuccess?.(); } catch(e:any){ setError(e?.message ?? "Failed to sign up"); } finally { setLoading(false); }
  };
  return (
    <form className="mt-4 grid gap-3" onSubmit={(e)=>{e.preventDefault(); void onSubmit();}}>
      ...
      <button type="submit" disabled={loading} className="inline-flex h-9 items-center justify-center rounded-md bg-black px-3 text-white disabled:opacity-50">{loading?"Creating…":"Create account"}</button>
    </form>
  );
}
